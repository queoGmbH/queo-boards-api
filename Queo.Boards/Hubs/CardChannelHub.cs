...
namespace Queo.Boards.Hubs {
    /// <summary>
    ///     Hub, das verwendet wird um Karten spezifische Events per SignalR zu publishen/pushen.
    ///     Die Beschränkung auf genau eine Karte erfolgt beim Anmelden über den Query-Parameter "card", in welchem die
    ///     <see cref="Card.BusinessId">BusinessId</see> der Karte enthalten sein muss.
    ///     Wird keine BusinessId übergeben, kann keine Anmeldung erfolgen.
    /// </summary>
    public class CardChannelHub : GenericHub {
        protected static bool TryGetChannel(HubCallerContext context, out string channel) {
            channel = context.QueryString["card"];
            if (channel != null) {
